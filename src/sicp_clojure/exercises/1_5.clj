...
;; In applicative order evaluation operands in a combination are evaluated before the operators, so (p) will be evaluated before (evaluation-order-test).
;; In scheme this tests would rerun infinately because it is an infinately recursive process.
;; In Clojure blows the stack because tail calls are not optimised (need to use recur to get a similar effect).
(deftest ex_1.5
...
;; In this case the predicate will evaluate to true and so the alternative expression is never evaluated and the infinitely recursive procedure p is never evaluated.